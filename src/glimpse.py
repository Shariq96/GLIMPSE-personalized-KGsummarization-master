...
class SummaryMethod(object):
    ...
    def __call__(self, user, KG, K, query_log):
        """
        :param KG: KnowledgeGraph
        :param K: summary constraint
        :param query_log: query log
        :param user: no of user
        :return results: results of function call
        """
        return self.fn_(user, KG, K, query_log, **self.kwargs_)


def GLIMPSE(user, KG, K, query_log, epsilon=1e-3, power=1):
    """
